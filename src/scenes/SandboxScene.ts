...
const lightSpriteSheet = new SpriteSheet(require('../anim/light.json'));
const fireSpriteSheet = new SpriteSheet(require('../anim/fire.json'));
...
class SandboxScene extends Node {
	...
	public enter() : void {
		...
		const instructionsLabel = new Text({x:200, y: 100}, 'Movement: WASD keys', {fillStyle: '#fff', font: 'sans-serif', fontSize: 16, textAlign: 'center'});
		...
		const fire = new AnimatedSprite(
			{x: 640, y: 640},
			{x: 0, y: 0},
			fireSpriteSheet.get([0]),
			AssetsManager.instance.getImage('fire'),
			{
